(function(){const s=document.createElement("link").relList;if(s&&s.supports&&s.supports("modulepreload"))return;for(const t of document.querySelectorAll('link[rel="modulepreload"]'))o(t);new MutationObserver(t=>{for(const i of t)if(i.type==="childList")for(const a of i.addedNodes)a.tagName==="LINK"&&a.rel==="modulepreload"&&o(a)}).observe(document,{childList:!0,subtree:!0});function r(t){const i={};return t.integrity&&(i.integrity=t.integrity),t.referrerPolicy&&(i.referrerPolicy=t.referrerPolicy),t.crossOrigin==="use-credentials"?i.credentials="include":t.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function o(t){if(t.ep)return;t.ep=!0;const i=r(t);fetch(t.href,i)}})();console.log("JavaScript loading...");const w=document.querySelector("#app");w?(console.log("Setting innerHTML..."),x()):(console.error("App element not found!"),document.body.innerHTML="<h1>Error: App element not found</h1>");function x(){w.innerHTML=`
  ...
  </div>`,k()}let y={};function u(e){return y[e]||(y[e]=document.getElementById(e)),y[e]}function k(){requestAnimationFrame(()=>{P(),L()}),setTimeout(()=>{j(),q(),D(),C(),S()},100)}function S(){const e=document.getElementById("basic-contact-form");if(!e)return;const s=()=>window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1"?"http://127.0.0.1:8787/api/forms":"https://hldesignedit.hligon.workers.dev/api/forms",r=document.getElementById("basic-contact-status"),o=document.getElementById("contact-name"),t=document.getElementById("contact-email"),i=document.getElementById("contact-message"),a=c=>{r&&(r.textContent=c)};e.addEventListener("submit",c=>{c.preventDefault();const d=new FormData(e),f=String(d.get("name")||"").trim(),l=String(d.get("email")||"").trim(),p=String(d.get("phone")||"").trim(),v=String(d.get("subject")||"").trim(),h=String(d.get("message")||"").trim();if(!f){a("Please enter your name."),o&&o.focus();return}if(!l||!/^\S+@\S+\.\S+$/.test(l)){a("Please enter a valid email address."),t&&t.focus();return}if(!h){a("Please enter a message."),i&&i.focus();return}const b=s(),m=e.querySelector('button[type="submit"]');m&&(m.disabled=!0),a("Sending..."),fetch(b,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({formType:"contact",honeypot:String(d.get("company")||""),pageUrl:window.location.href,data:{name:f,email:l,phone:p,subject:v,message:h}})}).then(async g=>{const n=await g.json().catch(()=>null);if(!g.ok||!(n!=null&&n.ok))throw new Error((n==null?void 0:n.error)||"Unable to send message.");a("Message sent! I will reply within 24 hours."),e.reset()}).catch(()=>{a("Could not send right now. Please email info@hldesignedit.com.")}).finally(()=>{m&&(m.disabled=!1)})})}function j(){const e=u("footer-year");e&&(e.textContent=new Date().getFullYear().toString())}function P(){const e=document.querySelectorAll(".filter-btn"),s=document.querySelectorAll(".portfolio-card");!e.length||!s.length||document.querySelector(".portfolio-filters").addEventListener("click",r=>{if(!r.target.classList.contains("filter-btn"))return;e.forEach(t=>t.classList.remove("active")),r.target.classList.add("active");const o=r.target.dataset.filter.trim().toLowerCase();requestAnimationFrame(()=>{s.forEach(t=>{const i=(t.dataset.type||"").trim().toLowerCase(),a=o==="all"||i===o;t.style.display=a?"":"none"})})},{passive:!0})}function L(){document.addEventListener("click",e=>{const s=e.target.closest('a[href^="#"]');if(!s)return;e.preventDefault();const r=s.getAttribute("href"),o=document.querySelector(r);if(o){const a=o.offsetTop-80;window.scrollTo({top:a,behavior:"smooth"})}},{passive:!1})}function q(){const e=u("greeting-text");if(!e)return;const s=["Hola","Bonjour","Ciao","こんにちは","你好","Hello"];let r=0;const o=()=>{e.style.opacity="0",setTimeout(()=>{e.textContent=s[r],e.style.opacity="1",r=(r+1)%s.length},200)};e.style.transition="opacity 0.2s ease",setTimeout(()=>{o(),setInterval(o,3e3)},2e3)}function D(){const e=document.querySelector(".portfolio-header");if(!e)return;let s=!1;const r=()=>{const o=window.scrollY>100;e.style.background=o?"rgba(255, 255, 255, 0.98)":"rgba(255, 255, 255, 0.95)",e.style.boxShadow=o?"0 4px 6px -1px rgba(0, 0, 0, 0.1)":"",s=!1};window.addEventListener("scroll",()=>{s||(requestAnimationFrame(r),s=!0)},{passive:!0})}function C(){const e=u("project-modal"),s=u("project-request-btn"),r=u("modal-close"),o=u("modal-cancel"),t=u("project-form"),i=u("other-project-type"),a=e==null?void 0:e.querySelector(".modal-btn-primary");if(!e||!s||!t||!a)return;const c=()=>{e.style.display="none",document.body.style.overflow="auto",t.reset(),i&&i.classList.remove("active")};s.addEventListener("click",l=>{l.preventDefault(),e.style.display="flex",document.body.style.overflow="hidden"}),r&&r.addEventListener("click",c),o&&o.addEventListener("click",c),e.addEventListener("click",l=>{l.target===e&&c()}),document.addEventListener("keydown",l=>{l.key==="Escape"&&e.style.display==="flex"&&c()}),t.addEventListener("change",l=>{if(l.target.name==="project-type"){const p=l.target.value==="other"&&l.target.checked;if(i){i.classList.toggle("active",p);const v=i.querySelector("input");v&&(v.required=p,p||(v.value=""))}}},{passive:!0});const d=t.querySelector('textarea[name="finalNotes"]'),f=t.querySelector(".char-counter");d&&f&&d.addEventListener("input",()=>{const l=d.value.length;f.textContent=`${l} / 1000 characters`,f.classList.toggle("warning",l>800)},{passive:!0}),t.addEventListener("submit",async l=>{l.preventDefault();const p=a.innerHTML,v=a.style.background;a.disabled=!0,a.innerHTML='<span class="loading-spinner"></span> Submitting...';const b=window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1"?"http://127.0.0.1:8787/api/forms":"https://hldesignedit.hligon.workers.dev/api/forms";try{const m=Object.fromEntries(new FormData(t).entries()),g=await fetch(b,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({formType:"project",honeypot:String(m.company||""),pageUrl:window.location.href,data:m})}),n=await g.json().catch(()=>null);if(!g.ok||!(n!=null&&n.ok))throw new Error((n==null?void 0:n.error)||"Unable to submit request.");a.innerHTML="✓ Request Submitted!",a.style.background="linear-gradient(135deg, #059669, #10b981)",setTimeout(()=>{alert("Thank you! Your project request has been submitted successfully. I will review it and get back to you within 24 hours."),c(),a.innerHTML=p,a.style.background=v,a.disabled=!1},800)}catch{a.innerHTML="Error - Please Try Again",a.style.background="linear-gradient(135deg, #dc2626, #ef4444)",setTimeout(()=>{a.innerHTML=p,a.style.background=v,a.disabled=!1},2500)}})}
